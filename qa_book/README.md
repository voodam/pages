...
Эта книга для тех, кто хочет начать карьеру QA без какого-либо бэкграунда. Подойдет и для программиста, но в меньшей степени, так как от него обычно требуются более глубокие знания. Подойдет и просто для интересующихся.

В интернете огромное количество информации, курсов, книг и статей, однако мне кажется, что есть недостаток ресурсов для стопроцентных новичков. Хотелось объяснить все самое главное, излишне не пускаясь в технические подробности. Я постарался написать книгу так, чтобы она была хорошей отправной точкой, включающей все нужные темы, но при этом оставаясь насколько возможно компактной.
...
Книга выложена на Github, чтобы любой мог [поучаствовать](https://github.com/voodam/pages/issues/new) в ее развитии. Моя задача - сделать ее ясной для новичка, поэтому не стесняйтесь писать о том, что вы не поняли. Буду рад любым идеям, как можно улучшить это руководство. Поблагодарить можно [здесь](https://github.com/voodam/pages/).

## Понимание программ

### Что такое программа

IT - это про разработку и использование программ. Программы выполняют полезную работу и взаимодействуют друг с другом. Разные люди пишут их, тестируют, создают к ним дизайн, планируют развитие, управляют командами людей, которые все это делают и так далее. Поэтому нужно начать с понимания того, что такое программа.

Программа подобна математической функции: у нее есть ввод, вывод, и она производит какую-то работу. Например, у функции `f(x) = x^2` ввод - это число x, вывод - это x в квадрате. Для числа 2 она выдаст 4, для 3 - 9 и так далее. Программы делают то же самое, но работают не только с числами, а также со строками, графическими объектами, различного вида данными. Например, компьютерная игра - это программа, у которой ввод - это движения мыши и нажатия клавиатуры, а вывод - это графические изображения на экране. Внутри она раскладывается на множество компонентов, которые выполняют сложную работу, но сути это не меняет. У программы может быть не один, а несколько вводов и выводов. Например, другая многопользовательская игра принимает движения мыши, клавиатуры, данные по сети от других игроков, а выдает изображение, пересылает данные прочим игрокам по сети и пишет в файл на жестком диске вспомогательную информацию.
...
* Движения мыши и нажатия клавиатуры (если перед нами программа-автокликер)
* Файл
...
В общем, как вы поняли, для программ то, что является выводом, может быть и вводом, все зависит от задач, которые они выполняют. Ввод - это то, от чего отталкивается программа при генерации вывода. Программа не обязана иметь графический интерфейс, а может быть сугубо консольной (это обычное дело для IT софта, но об этом позже).

Программы существуют в трех состояниях:
1. Исходные коды - текстовые файлы, из которых создаются исполняемые файлы (состояние 2).
2. Файл или набор исполняемых файлов на жестком диске (или на оптическом диске, флешке, или любом другом носителе). В таком виде программа не работает, а просто хранится, чтобы быть в будущем вызванной.
3. Запущенная программа, которая выполняет работу (называется процессом). Она загружается операционной системой с диска в оперативную память и исполняется.

При этом исполнение может происходить в двух режимах:
1. Программа не выключается, пока ей каким-то образом об этом не сказать (например, нажать крестик в углу приложения).
2. Программа завершит работу, как только выполнит свою задачу (так работают многие консольные утилиты).

После завершения работы программа удаляется из оперативной памяти и остается храниться только на диске (2).

### Трансляторы

Трансляторы - это программы, которые нужны для того, чтобы конвертировать программы из исходных кодов (1) в исполняемые файлы (2). Трансляторы бывают двух видов: интерпретаторы и компиляторы (опустим пример Java, С# и других подобных языков, где используется нечто среднее). Для интерпретаторов исходные коды также являются и исполняемыми файлами (1 = 2), то есть не создается дополнительных файлов, а исполняются исходные коды при каждом запуске программы. Компиляторы же по-честному создают исполняемые файлы. Для нас это различие не важно - просто полезно, чтобы не запутаться в терминах. Важно понимать цепочку: программист написал исходный код - транслятор создал из них исполняемые файлы - операционная система загрузила исполняемые файлы - программа работает.

### Язык программирования

Исходные коды пишутся на языке каком-либо программирования. Язык программирования - это специальный язык, который достаточно формален, чтобы из него мог создать исполняемый файл транслятор, но при этом достаточно понятен для человека, чтобы людям было комфортно выражать на нем свои идеи. Существует много разных языков для разных целей:
...
* C/C++ (компилируемый) - написание системного ПО и высокопроизводительных приложений (на самом деле это не один язык, а два, при этом C++ является надмножеством C). Другими словами, написание таких приложений, где низкая скорость выполнения неприемлема и где необходимы низкоуровневые возможности. Его намного сложнее учить, медленнее скорость разработки и он более чувствителен к ошибкам программиста. Поэтому на C/C++ не пишут все подряд, а используют для этого языки вроде тех, что выше.
* Множество других языков для разных задач и с разными подходами, разной степени популярности.

Почему существует так много языков программирования? Отчасти из-за существования разных задач, отчасти по историческим причинам, отчасти из-за желания реализовать те или иные подходы в программировании или по соображениям конкуренции. Но вас, вероятно, будет интересовать какой-то язык из первых четырех пунктов. Хороший вариант - Python.

В принципе, язык можно использовать и в нетипичной для него нише, если это зачем-то нужно. Но при этом стоит быть готовым к дополнительным трудностям (например, что-то не будет работать так, как должно, или разработка займет больше времени).

### Командная оболочка

Запускается как `cmd.exe` или Командная строка в Windows, а в Linux обычно называется Терминал. Это просто альтернативный инструмент ввода информации, к которому надо привыкнуть. Ничего принципиально сложного в нем нет, но сперва может быть непривычно. Зато, когда вы освоитесь, появится ощущение удобства и богатые возможности автоматизации рутинных задач (чего сложно добиться для графических приложений). Команды вводятся клавиатурой, а мышь или не работает, или имеет ограниченное применение. Необходимо приобрести некоторые навыки работы с командной оболочкой, чтобы эффективно выполнять рабочие задачи. Далее в книге будет руководство по работе с оболочкой в [Linux](#работа-в-linux).


## Элементы языков программирования

Языки программирования имеют в своей основе много общего. Не нужно пугаться обилия подробностей при чтении учебников, пытаться выучить все тонкости языка и имена функций в стандартной библиотеке. Чтобы начать работать, нужно знать не так уж много. Лучше всего изучать язык на практике. Общее правило: если что-то непонятно, запустите интерпретатор и поэкспериментируйте.
...
WebSocket - протокол для полноценного двустороннего (полнодуплексного) взаимодействия браузера и сервера. Назван, по-видиму, по аналогии с socket, но для Web. Изначально он создавался именно как браузерная технология, так как обычные приложения (а не веб-сайты) могли использовать обычные сокеты.
...
* Сайт на основе HTTP-запроса выбирает, что ему нужно сделать. Например, показать определенную HTML страницу, сгенерированную на основе данных, введенных перед этим другими пользователями и сохраненных в базе данных. Или, если это запрос на сохранение, то сохранить эти данные, если они введены корректно.
...
На мой взгляд, парадигма, которая создает так много проблем и дает при этом не так много выгод, не может считаться успешной. Но вообще, это касается любых возведенных в абсолют парадигм и философских идей (в том числе функционального программирования). Как всегда, вся суть в трейд-оффах, а серебряной пули нет.

Краткие выводы:

* ООП в исконном виде устарела, но все еще оказывает большое влияние на разработку и будоражит умы многих. Постарайтесь избавиться от идеологического компонента и использовать подход себе на благо. Для этого полезно опираться на "три столпа": нужно ли мне наследование, инкаспуляция или полиморфизм? Если да, то вероятно нужна и ООП. Не стесняйтесь смешивать процедурный и ООП подходы в одном проекте.

* Порой ООП - сам по себе органичный способ абстрагирования кода: есть набор процедур, общий набор переменных, с которым они рабоают, а также ясная, "напрашивающаяся" идея абстракции.

* Вокруг некоторых языков программирования сформировано ООП-ориентированное сообщество, от которого никуда не деться (Java, C#, PHP). В нем ООП используется повсеместно (не создаются отдельные процедуры вне классов), но оно довольно сильно переосмыслено и де-факто это тоже смешанная парадигма, в которой компенсированы негативные факторы. Описание этого подхода выходит за рамки этой книги.
...
Эти знания позволят вам подняться на новый уровень: QA, умеющий хорошо программировать и проектировать, считается сильным специалистом уровня Middle-Senior. Как обычно, важно больше практиковаться. Не бойтесь брать на себя задачи на программирование на работе - они составят одни из наиболее ценных строк в резюме.
...
def wait_until(condition, timeout=10, poll_frequency=0.5):
  ...
  # Интересно, что, так как обработчик сработает по таймеру, то исключение будет
  # выброшено из обработчика (лямбды) выше как из точки входа.
...
* Чтобы не вводить пароль при каждом запуске `ssh`, добавьте эту строчку в `~/.bashrc`: `[ -z "$SSH_AUTH_SOCK" ] && eval "$(ssh-agent -s)"`. Теперь пароль нужно вводить только однажды на время работы системы.
...
Желаю успехов, и не бойтесь ошибаться!
